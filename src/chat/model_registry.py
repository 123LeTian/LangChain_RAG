"""Model shelf configuration for the chat platform — multi-provider with env placeholder resolution."""
...
# 加载 .env 文件以便模型配置可以引用其中的变量
_ENV_LOADED = False


def _ensure_dotenv() -> None:
    """在首次使用前加载项目根目录的 .env 文件。"""
    global _ENV_LOADED
    if _ENV_LOADED:
        return
...
def _resolve_placeholders(value: Any) -> Any:
    ...
    if isinstance(value, str):
        ...
        if match:
            ...
            if raw is not None:
                return _cast_resolved_value(raw)
            if default is not None:
                return _cast_resolved_value(default)
            return value
        def _replacer(m: re.Match) -> str:
            ...
            if dv is not None:
                return dv
            return m.group(0)
        return _ENV_PLACEHOLDER_RE.sub(_replacer, value)
    return value


_CAST_BOOL = {"true": True, "false": False}


def _cast_resolved_value(raw: str) -> Any:
    """将环境变量字符串转为合适的 Python 类型。"""
    lowered = raw.strip().lower()
    if lowered in _CAST_BOOL:
        return _CAST_BOOL[lowered]
    if lowered.isdigit():
        return int(lowered)
    try:
...
class ModelRegistry:
    ...
    def _load_models(self) -> List[ChatModel]:
        ...
        if self.config_path.exists():
            ...
            builtins = [
                ChatModel(**_resolve_placeholders({**item, "is_builtin": True}))
                for item in models
                if isinstance(item, dict)
