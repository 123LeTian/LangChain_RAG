...
上传接口会把文档元数据写入 `data/knowledge_bases.json`，把真实文件写入 `documents/<kb_id>/<doc_id>__<filename>`，并在文档记录里保存 `storage_path`。`filename` 只作为前端展示和引用来源名称；RAG 初始化、Graph 构建、预览、删除都会通过统一的 `storage_path` resolver 找真实文件。

旧数据仍兼容只包含 `filename` 的记录：系统会回退读取 `documents/<filename>`。新的上传不要再手动写扁平路径，避免不同知识库里同名文件互相覆盖。删除文档或知识库时，后端只会删除当前记录独占的文件，不会删除仍被其他知识库记录引用的文件。
...
新上传文档应同时具备 `filename` 和 `storage_path`。如果某个知识库显示 `1 文档 · 0 切片`，先打开 `data/knowledge_bases.json` 确认该文档的 `kb_id` 是否等于当前知识库 ID，再确认 `documents/<storage_path>` 或旧格式 `documents/<filename>` 是否真实存在。同名文件可以出现在不同知识库中，因为真实路径已经按 `kb_id/doc_id` 隔离。
