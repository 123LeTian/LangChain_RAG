...
需要注意：五种 RAG 的策略代码和测试都存在，但当前真实接口 `/api/rag/query` 与 `/api/rag/query/stream` 主要由 `RealRAGService` 承接真实文档检索链路。Graph 页面、Trace 查询页和 Evaluation 页面仍保留部分演示/mock 数据，后续如果要做最终答辩级验收，需要继续打通五种 mode 到对应 strategy 的真实端到端路由和评测报告。
...
| 模式 | 当前状态 | 说明 |
| --- | --- | --- |
| Naive RAG | 基本完成 | 文档加载、切分、向量检索、生成、引用和 trace |
| Advanced RAG | 基本完成 | Query rewrite、multi-query、hybrid retrieval、rerank、compression |
| Modular RAG | MVP 完成 | 支持 rewrite / retrieve / rerank / compress / verify 模块开关和配置校验 |
| GraphRAG | MVP 完成 | 实体、关系、社区报告、local/global graph retrieval 代码存在 |
| Agentic RAG | MVP 完成 | Agent workflow、tool registry、vector/graph/summary tools 存在 |
...
- 五种 RAG strategy 已存在，但真实 `/api/rag/query` 入口还需要继续强化 mode 到 strategy 的端到端路由。
